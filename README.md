...
* **Mention Filtering**: Find issues where a user is referred to, in the description
  or in any note you are allowed to read. Both of Redmine's forms count: `@login`,
  which notifies, and `user#176`, which renders as a link to that profile and
  notifies nobody.
    * `mentioned` (mentioned or linked)
    * `involved_or_mentioned` (assignee, author, watcher, mentioned or linked)
...
Matching Redmine's `user#<id>` link form alongside `@login` costs a second prefilter.
Measured on the same dataset with 20 principals selected, and with the link form
actually present in 2 000 journals:

| forms matched | PostgreSQL 16 | MySQL 8.0 |
|---|---|---|
| `@login` only | 241 ms | 201 ms |
| `@login` and `user#<id>` (shipping) | 332 ms | 226 ms |
| both, plus `user:<login>` (not shipped) | 506 ms | 234 ms |

On PostgreSQL the widened filter found 1 305 issues where the narrow one found 436, so
the extra cost buys three times the answer. The third row is why `user:<login>` is not
matched: it forces the prefilter from `LIKE '%user#%'` to `LIKE '%user%'`.
...
**Two forms, one filter.** Redmine has two ways of pointing at a person in text, and
they are not the same thing:

* `@jsmith` is a **mention**. `Redmine::Acts::Mentionable` scans for it on save and
  notifies that user.
* `user#176` is a **link**. `application_helper` renders it as a link to the profile;
  `MENTION_PATTERN` does not know it and nobody is notified.

The filter matches both, because people write both meaning the same thing. So it
answers *"where is this person referred to"*, which is wider than *"who was
notified"* — the label says "mentioned or linked" rather than "mentioned" for exactly
that reason. If you need only the notifying form, there is no filter for it; that
distinction lives in the notification log, not in the text.

`user##176` counts too, since Redmine renders that as the same link. Redmine's third
form, `user:jsmith`, is **not** matched: it would force the cheap prefilter from
`LIKE '%user#%'` to `LIKE '%user%'`, and measured on 20 000 issues that took
PostgreSQL from 332 ms to 506 ms for a form nobody had asked for.

**Accuracy.** Matching happens in two stages. A cheap `LIKE '%@%' OR LIKE '%user#%'`
throws away every text that can hold neither form, then one regular expression carries
all the selected logins and ids and applies the boundaries that `LIKE` cannot express. `@jan` therefore does
...
The link form is bounded by Redmine's own `LINKS_RE` rather than by a rule of our own:
`user#17` does not match `user#176`, `user#176x` is not a link at all, and
`superuser#176` is not one either. One divergence is deliberate — `LINKS_RE` carries no
`/i`, so Redmine does not link `USER#176` while this filter matches it, because the
login half of the same pattern has to be case insensitive.

A user whose account has no login at all — possible on accounts created before Redmine
required one — can now be found by `user#<id>`, where previously the filter had nothing
to look for and matched nothing.
